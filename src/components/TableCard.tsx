...
import { Edit, Trash2, Users, UserMinus, AlertTriangle, ChevronDown, ChevronRight } from "lucide-react"
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
      <div className="p-4 border-b border-gray-200">
        ...
        {/* Toggle button for guest list */}
        {table.guests.length > 0 && (
          <button
            onClick={() => setIsExpanded(!isExpanded)}
            className="mt-3 flex items-center space-x-2 text-sm text-gray-600 hover:text-gray-800 transition-colors w-full"
          >
            {isExpanded ? (
            ...
            <span>{isExpanded ? 'Hide' : 'Show'} guest list ({table.guests.length})</span>
          </button>
        )}
      </div>

      {/* Guest list - only show when expanded or when empty */}
      {(isExpanded || table.guests.length === 0) && (
        <div className="p-4">
          {table.guests.length === 0 ? (
